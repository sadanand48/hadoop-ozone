...
/**
 * Ledger row recording trapped accounting ownership for a deleted entry.
 */
public final class SnapshotTrappedLedgerEntry {
  ...
  public static String encodeKey(long volumeId, long bucketId, String deleteDbKey) {
    String normalizedDeleteKey =
        normalizeDeleteDbKey(Objects.requireNonNull(deleteDbKey, "deleteDbKey == null"));
    return new StringBuilder()
        .append(OM_KEY_PREFIX).append(volumeId)
        .append(OM_KEY_PREFIX).append(bucketId)
        .append(OM_KEY_PREFIX).append(normalizedDeleteKey)
        .toString();
  }

  private static String normalizeDeleteDbKey(String deleteDbKey) {
    return deleteDbKey.startsWith(OM_KEY_PREFIX)
        ? deleteDbKey.substring(1)
        : deleteDbKey;
  }
